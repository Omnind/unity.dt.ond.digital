...
namespace u040.prespective.prelogic.logiccontrollers.kinetics.editor
{
    ...
    [CustomEditor(typeof(LEGACY_ServoMotorLogicController))]
    public class LEGACY_ServoMotorLogicControllerEditor : PrespectiveEditor
    {
        private LEGACY_ServoMotorLogicController component;
        ...
        {
            component = (LEGACY_ServoMotorLogicController)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("No Servo Motor Component has been set. This component will not function properly untill all required components have been assigned. You can do this in the Properties tab.", MessageType.Warning);
            }
            ...
            switch (component.CurrentTab)
            {
                case "Properties":
                    if (Application.isPlaying) //Make sure motor physical properties cannot be editted during playmode
                    {
                        if (component.ServoMotor == null)
                        {
                            EditorGUILayout.LabelField("No Servo Motor has been set so no properties can be shown.");
                        }
                        else
                        {
                            EditorGUILayout.LabelField("Servo Motor", component.ServoMotor.ToString());
                        }
                    }
                    else
                    {
                        component.ServoMotor = (LEGACY_ServoMotor)EditorGUILayout.ObjectField("Servo Motor", component.ServoMotor, typeof(LEGACY_ServoMotor), true);
                    }
                    break;
